...
class Instance(object):
    ...
    def compute_vrp_routes(self, nodes, node_demand, capacity):
        routes_list = []
        capacity_used = 0
        count = 1

        for i in nodes:
            if (capacity_used + node_demand[i]) <= capacity:
                capacity_used += node_demand[i]
                if i is nodes[-1]:
    ...
    def compute_vrp_distance(self, routes, nodes, matrix):
        starts = 0
        distance = 0
        for i in routes:
            total = 0
            total += matrix[0][nodes[starts]]
            cont = starts
            while cont < starts + (i - 1):
                total += matrix[nodes[cont]][nodes[cont + 1]]
                cont += 1
            total += matrix[nodes[starts + (i - 1)]][0]
            starts += i
            distance += total
        return distance

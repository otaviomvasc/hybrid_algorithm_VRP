...
class Individuo(object):
    ...
    def __init__(self, new_ind=None, size=None, change_vel=None):
            ind = new_ind
            position = [random.uniform(0,1) for i in range(len(ind))]
    ...
    def order_per_position(self, ind=None, position=None):
        if ind is not None and position is not None:
            ...
            self.velocity = np.array([self.velocity[i] for i in indices])

    def fitness(self):
        return self.fit

    def velocity_calculation(self, best, c1, c2,r1,r2):
        self.velocity = 1.2 * self.velocity + c1  * r1 * (self.best_local.position - self.position) + c2 * r2* (best.position - self.position)
...
if __name__ == "__main__":
    ...
    print(population[0].velocity)
    print(population[0].fitness())

    while i < iterations:
        r1 = random.uniform(0,1)
        r2 = random.uniform(0,1)
        #aumentar velocidade, dando mais peso para social, diminuir a vizinhança (modelo anel)
